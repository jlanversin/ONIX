...
import os
import shutil
from openbu.data import time_dic
...
def openmc_name_to_openbu_name(name):
	...
	openbu_name = name[:i] + '-' + am



	return openbu_name

def openbu_name_to_openmc_name(name):
...
def bu_namelist_to_mc_namelist(name_list):
	...
	new_name_list = []
	for name in name_list:
		new_name = openbu_name_to_openmc_name(name)
...
def mc_namelist_to_bu_namelist(name_list):
	...
	new_name_list = []
	for name in name_list:
		new_name = openmc_name_to_openbu_name(name)
		new_name_list.append(new_name)
...
def get_zamid_natural_abundance(zamid):
	...
	name_new_format = openbu_name_to_openmc_name(name_old_format)
...
def get_name_natural_abundance(name):
	...
	name_new_format = openbu_name_to_openmc_name(name)
	nat_abun_dict = d.NATURAL_ABUNDANCE
	if name_new_format in nat_abun_dict:
		nat_abun = d.NATURAL_ABUNDANCE[name_new_format]
	else:
