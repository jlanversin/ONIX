...
class Sequence(object):
    ...
    def __init__(self, id_number):
        ...
        self._id = id_number
        self._step_vector = None
        self._step_unit = None
    ...
    def _set_from_input(self, sequence_dict, passlist,  bu_sec_conv_factor):
        ...
        self.substeps_vector = sequence['substeps_vector']
    ...
    def _cell_conversion(self, passlist, bu_sec_conv_factor, mode):
        ...
        #                 substeps = self._substeps_vector[i]
        ...
        # elif self._step_unit == 'MWd/kg':

        #     self._bu_seq += self._step_vector
        #     for i in range(len(self._bu_seq)-1):
        ...
        #         flux_list = [flux_value]*self._substeps_vector[s]
        ...
        #             power_list = [power_value]*self._substeps_vector[step]
    ...
    def _initial_system_conversion(self, system):
        ...
        # Directly create the whole time seq and subseq from step_vector and substeps
        if self._step_unit in ['s', 'm', 'y', 'd']:
            # Create the time sequence
            # += because self._time_seq is already initialised
            conv_to_sec = data.time_dic[self._step_unit]
            self._time_seq += [x*conv_to_sec for x in self._step_vector]
            for s in range(len(self._time_seq)-1):
                substeps = self._substeps_vector[s]
                #time_intvl += [self._time_seq[i+1]- self._time_seq[i]]
                time_intvl = self.get_time_intvl(s+1)
                time_subintvl_val = time_intvl/substeps
                self._time_subseq_mat.append([time_subintvl_val*(j+1) + self._time_seq[s] for j in range(substeps)])
            ...
            if norma_mode == 'flux':
                ...
                for s in range(len(self._system_bu_seq)-1):
                    substeps = self._substeps_vector[s]
                    self._system_bu_subseq_mat.append([0 for j in range(substeps)])

        elif self._step_unit == 'MWd/kg':
        # Directly create the whole system_bu seq and subseq from step_vector and substeps
            self._system_bu_seq += self._step_vector
            for s in range(len(self._system_bu_seq)-1):
                substeps = self._substeps_vector[s]
                system_bu_intvl = self.get_system_bu_intvl(s+1)
                system_bu_subintvl_val = system_bu_intvl/substeps
                #self._system_bu_subseq.append([system_bu_substep_val*(j+1) + self._system_bu_seq[i] for j in range(substeps)])
                self._system_bu_subseq_mat.append([system_bu_subintvl_val*(j+1) + self._system_bu_seq[s] for j in range(substeps)])
                #self._system_bu_subintvl.append([system_bu_substep_val for j in range(substeps)])
    ...
    #         if ss == substeps - 1: # Time to  update bu_step and bu_seq
    ...
    #         if i == self._substeps - 1: # Time to  update time_step and time_seq
    ...
    def initial_system_time_bu_conversion(self, system):
        ...
        substeps_vector = self._substeps_vector
    ...
    def _set_flux(self, new_flux, s, ss):
        ...
        # if self._norma_unit == 'power':
        #     substeps = self.substeps(s)
        #     if i == substeps - 1: # Time to  update flux_seq
    ...
    def _set_subpower(self, new_subpower, ss):
        ...
        # if self._norma_unit == 'flux':
        #     substeps = self.substeps(s)
        #     if i == substeps - 1: # Time to  update power_seq
        #         self._power_seq.append(power)
...
##### steps and norma info ######
    ...
    @property
    def substeps_vector(self):

        return self._substeps_vector

    @substeps_vector.setter
    def substeps_vector(self, substeps_vector):

        self._substeps_vector = substeps_vector
    ...
    def substeps(self, s):
        ...
        return self._substeps_vector[s]

    def set_step(self, step_vector, step_unit):

        self.step_vector = step_vector
        self.step_unit = step_unit
        self.steps_number = len(step_vector)
...
######## Set Step values
    ...
    # After the completion of a burn step
    # Add time to bu_seq and add the current bu_subseq to subseq_mat
    def _set_step_bucell_bu(self):
    ...
    # There are no substep MC_flux, therefore, set_step_MC_flux should
    # be the one to update the current MC_flux
    def _set_step_MC_flux(self, MC_flux):
    ...
    # There are no substep MC_flux, therefore, set_step_MC_flux should
    # be the one to update the current MC_flux
    def _set_step_flux_spectrum(self, flux_spectrum):
    ...
    # After an MC simulation
    # There are no substep kinf (as least calculated by MC), therefore, set_step_kinf should
    ...
    # After an MC simulation
    def _set_step_pow_dens(self, pow_dens):
    ...
    def _set_step_isomeric_branching_ratio(self, isomeric_branching_ratio):
